        ...
        <form onSubmit={handleSearch} className="max-w-2xl mx-auto">
          <div className="relative">
            <input
              ...
              className={`w-full pl-12 pr-28 py-4 ${styles.inputBg} rounded-2xl placeholder-gray-500 dark:placeholder-gray-500 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all text-lg`}
              autoFocus
            />
      ...
      <div className="mt-8">
        {loading ? (
          <div className="flex items-center justify-center py-20">
            ...
            <span className={`ml-2 ${styles.textSecondary}`}>Поиск...</span>
          </div>
        ) : query && !loading ? (
          <>
            <div className="mb-6">
            ...
            {totalResults === 0 ? (
              <div className="text-center py-20">
                ...
                <p className={`${styles.textSecondary} text-lg`}>Ничего не найдено</p>
                <p className={`${styles.textMuted} text-sm mt-2`}>Попробуйте изменить запрос</p>
              </div>
            ) : (
                    ...
                    <div className="grid gap-4">
                      {articles.map((item, idx) => (
                        <Link
                          key={idx}
                          href={item.href}
                          className={`group ${styles.cardBg} rounded-xl p-4 border hover:border-primary/30 transition-all hover:-translate-y-0.5 flex gap-4`}
                        >
                          {item.image && (
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              ...
                              <span className={`text-xs ${styles.textMuted} flex items-center gap-1`}>
                            ...
                            <p className={`text-sm ${styles.textSecondary} mt-1 line-clamp-2`}>{item.description}</p>
                          </div>
                          <ArrowRight className={`w-5 h-5 ${isDark ? 'text-gray-500' : 'text-gray-400'} group-hover:text-primary group-hover:translate-x-1 transition-all flex-shrink-0`} />
                        </Link>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="grid gap-4">
                      {songs.map((item, idx) => (
                        <Link
                          key={idx}
                          href={item.href}
                          className={`group ${styles.cardBg} rounded-xl p-4 border hover:border-primary/30 transition-all hover:-translate-y-0.5 flex justify-between items-center`}
                        >
                          <div>
                            <h3 className={`font-semibold ${styles.textPrimary} group-hover:text-primary transition-colors`}>
                              {item.title}
                            </h3>
                            <p className={`text-sm ${styles.textSecondary}`}>{item.artist}</p>
                            <div className="flex items-center gap-3 mt-2">
                              <span className={`text-xs ${styles.textMuted}`}>{item.bpm} BPM</span>
          ...
          <div className="text-center py-20">
            <div className="text-6xl mb-4">🔍</div>
            <p className={`${styles.textSecondary} text-lg`}>Введите запрос для поиска</p>
            <p className={`${styles.textMuted} text-sm mt-2`}>Найдёте уроки, песни и многое другое</p>
          </div>
        )}
      </div>
    </>
      ...
      <section className="relative overflow-hidden pt-16 pb-8">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4">
          <div className={`flex items-center gap-2 text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'} mb-6`}>
            ...
            <span className={isDark ? 'text-gray-300' : 'text-gray-700'}>Поиск</span>
          </div>
